      ...
      <div className="bg-white rounded-2xl sm:rounded-3xl shadow-sm border border-gray-100 p-4 sm:p-6 md:p-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-6 mb-6 sm:mb-8">
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div>
              <h1 className="text-xl sm:text-3xl font-bold text-gray-900">Learning Resources</h1>
              <p className="text-gray-500 mt-0.5 sm:mt-1 text-xs sm:text-base">Study materials, documents, and video tutorials.</p>
            </div>
          </div>
          
          <div className="flex bg-gray-100 p-1 rounded-xl self-start sm:self-auto overflow-x-auto scrollbar-hide">
            <button
              onClick={() => setActiveTab('pdfs')}
              className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-6 py-2 sm:py-2.5 rounded-lg font-medium transition-all whitespace-nowrap text-xs sm:text-sm ${
                activeTab === 'pdfs' 
        ...
        {activeTab === 'pdfs' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 px-0 sm:px-0">
            {pdfs.map((resource) => (
                ...
                <div className="p-4 sm:p-6 flex flex-col flex-1">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3 w-full">
                      <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${resource.isArticle ? 'bg-purple-50 text-purple-500' : 'bg-blue-50 text-blue-500'}`}>
                        {resource.isArticle ? <FileText className="w-5 h-5 sm:w-6 sm:h-6" /> : <FileDown className="w-5 h-5 sm:w-6 sm:h-6" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          <h3 className="text-sm sm:text-lg font-bold text-gray-900 leading-tight line-clamp-2" title={resource.title}>{resource.title}</h3>
                          <button 
                            onClick={(e) => toggleFavorite(e, resource.id)}
                            className="p-1 hover:bg-gray-100 rounded-full transition-colors flex-shrink-0"
                          >
                            <Heart className={`w-4 h-4 sm:w-5 h-5 ${favorites.includes(resource.id) ? 'fill-red-500 text-red-500' : 'text-gray-400'}`} />
                          </button>
                        </div>
                        <div className="flex items-center gap-2 mt-1 flex-wrap">
                          ...
                          <span className="text-[10px] text-gray-500">{resource.type}</span>
                          {resource.rating !== undefined && (
                            <div className="flex items-center gap-1 text-[10px] font-medium text-yellow-600 bg-yellow-50 px-2 py-0.5 rounded-md group/rating relative">
                  ...
                  <p className="text-gray-600 text-xs sm:text-sm mb-4 sm:mb-6 flex-1 line-clamp-3" title={resource.description}>
                    {resource.description}
                  </p>
                  
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between mt-auto pt-4 border-t border-gray-100 gap-3">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full bg-gray-200 flex items-center justify-center text-[10px] font-bold text-gray-600">
                        {resource.author.charAt(0)}
                      </div>
                      <span className="text-[10px] sm:text-xs text-gray-500 truncate max-w-[150px]">{resource.author}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <a 
                        href={resource.url}
                        ...
                        className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 text-xs sm:text-sm font-medium text-gray-600 hover:text-gray-900 bg-gray-50 hover:bg-gray-100 px-3 py-2 rounded-lg transition-colors"
                      >
                      ...
                      <a 
                        ...
                        className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 text-xs sm:text-sm font-medium text-blue-600 hover:text-blue-700 bg-blue-50 hover:bg-blue-100 px-3 py-2 rounded-lg transition-colors"
                      >
        ...
        {activeTab === 'videos' && (
          <div className="flex flex-col lg:flex-row gap-4 sm:gap-8 bg-[#f8f9fa] p-3 sm:p-6 rounded-2xl sm:rounded-3xl">
            {/* Left Content */}
            <div className="flex-1 space-y-8">
              
              {/* Featured */}
              {featured && (
                <a href={featured.url} target="_blank" rel="noopener noreferrer" className="block relative rounded-2xl sm:rounded-3xl overflow-hidden aspect-video sm:aspect-[21/9] group shadow-sm">
                  ...
                  <div className="absolute top-3 sm:top-6 left-3 sm:left-6 flex gap-2 z-10">
                    <span className="px-3 sm:px-4 py-1 sm:py-1.5 bg-white/20 backdrop-blur-md text-white text-[10px] sm:text-sm rounded-full border border-white/10">
                      Featured
                    </span>
                  </div>
                  
                  <div className="absolute top-3 sm:top-6 right-3 sm:right-6 px-2 sm:px-3 py-1 sm:py-1.5 bg-white/20 backdrop-blur-md text-white text-[10px] sm:text-sm font-medium rounded-full flex items-center gap-1 border border-white/10">
                    {featured.rating?.toFixed(1) || '4.5'} <Star className="w-3 sm:w-3.5 h-3 sm:h-3.5 text-yellow-400 fill-current" />
                  </div>

                  <div className="absolute bottom-0 left-0 p-4 sm:p-8 w-full md:w-2/3">
                    <h2 className="text-xl sm:text-3xl md:text-4xl font-bold text-white mb-2 sm:mb-3 line-clamp-2">{featured.title}</h2>
                    <p className="hidden sm:block text-gray-300 text-sm md:text-base mb-6 line-clamp-2">{featured.description}</p>
                    
                    <div className="flex items-center gap-3 sm:gap-4">
                      <button className="flex items-center gap-2 bg-[#6366f1] hover:bg-[#4f46e5] text-white px-4 sm:px-6 py-2 sm:py-2.5 rounded-full text-sm font-medium transition-colors">
                        <Play className="w-4 h-4 sm:w-5 sm:h-5 fill-current" /> Watch
                      </button>
                      <button 
                        onClick={(e) => toggleFavorite(e, featured.id)}
                        className="w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-md flex items-center justify-center text-white transition-colors border border-white/10 z-10"
                      >
                        <Heart className={`w-4 h-4 sm:w-5 h-5 ${favorites.includes(featured.id) ? 'fill-red-500 text-red-500' : 'text-white'}`} />
                      </button>
                    </div>
                  </div>
                </a>
